...
async function processItem(
    ...
    try {
        ...
        if (mode === 'new') {
            await updateProjectItemStatus(item.id, STATUSES.techDesignReview);
...
async function main(): Promise<void> {
    ...
    if (options.id) {
        ...
        const newItems = await listProjectItems(STATUSES.readyForTechDesign, undefined, options.limit || 50);
