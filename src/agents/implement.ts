...
async function main(): Promise<void> {
    ...
    if (options.id) {
        ...
        if (item.status === STATUSES.implementation && !item.reviewStatus) {
            mode = 'new';
        } else if (item.status === STATUSES.implementation && item.reviewStatus === REVIEW_STATUSES.requestChanges) {
            mode = 'feedback';
            ...
            console.error(`  Expected: "${STATUSES.implementation}" with empty Review Status or Review Status "${REVIEW_STATUSES.requestChanges}"`);
            process.exit(1);
        }
        ...
        // Flow B: Fetch items needing revision (Implementation status with Request Changes)
        if (adapter.hasReviewStatusField()) {
