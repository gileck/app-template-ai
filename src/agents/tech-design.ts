 ...
 *
 * Flow A (New Design):
 *   - Fetches items in "Ready for Technical Design" status
 ...
 *   - Moves to "Technical Design Review" status
 *   - Sets Review Status to "Waiting for Review"
 *
 * Flow B (Address Feedback):
 *   - Fetches items in "Technical Design Review" with Review Status = "Request Changes"
...
async function processItem(
    ...
    try {
        ...
        if (options.dryRun) {
            ...
            console.log('  [DRY RUN] Would update status to Technical Design Review');
        ...
        if (mode === 'new') {
            await adapter.updateItemStatus(item.id, STATUSES.techDesignReview);
            console.log(`  Status updated to: ${STATUSES.techDesignReview}`);
        }
...
async function main(): Promise<void> {
    ...
    if (options.id) {
        ...
        if (item.status === STATUSES.readyForTechDesign) {
            mode = 'new';
        } else if (item.status === STATUSES.techDesignReview && item.reviewStatus === REVIEW_STATUSES.requestChanges) {
            mode = 'feedback';
        } else {
            ...
            console.error(`  Expected: "${STATUSES.readyForTechDesign}" or "${STATUSES.techDesignReview}" with Review Status "${REVIEW_STATUSES.requestChanges}"`);
            process.exit(1);
        }
        ...
        console.log(`\nFetching items in "${STATUSES.readyForTechDesign}"...`);
        const newItems = await adapter.listItems({ status: STATUSES.readyForTechDesign, limit: options.limit || 50 });
        ...
        if (adapter.hasReviewStatusField()) {
            ...
            const feedbackItems = await adapter.listItems({
                status: STATUSES.techDesignReview,
